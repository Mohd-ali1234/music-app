"""MusicBrainz metadata enrichment (background, best-effort).

A single-worker client that enriches song documents with canonical metadata
(album, genres, ISRC, release date, ...) without ever blocking playback.
Requests respect MusicBrainz's ~1 req/sec rate limit.

NOTE: This enricher is provided as an optional background service and is not
currently wired into any HTTP route (matching prior behavior). To enable it,
construct a :class:`MusicBrainzEnricher` at startup and call ``enqueue`` after
a song is materialized.
"""
...
class MusicBrainzEnricher:
    ...
    def __init__(self, songs, cache, user_agent):
        ...
        threading.Thread(
            target=self._work, daemon=True, name="musicbrainz-enrichment"
        ).start()

    def enqueue(self, song_id):
        song = self.songs.find_one(
            {"id": song_id}, {"metadata.enrichment_status": 1}
        )
        ...
        self.songs.update_one(
            {"id": song_id}, {"$set": {"metadata.enrichment_status": "queued"}}
        )
    ...
    def _request(self, path, params):
        ...
        url = (
            ...
            + path
            + "?"
            + urllib.parse.urlencode({**params, "fmt": "json"})
        )
    ...
    def _enrich(self, song_id):
        ...
        if cached and cached.get("result") is not None:
            ...
            query = (
                f'recording:"{song.get("title", "")}" '
                f'AND artist:"{song.get("artist", "")}"'
            )
            ...
            self.cache.update_one(
                {"key": key},
                {"$set": {"result": result, "fetched_at": _now()}},
                upsert=True,
            )
        ...
        if not result:
            self.songs.update_one({"id": song_id}, {"$set": {
                "metadata.enrichment_status": "no_match",
        ...
        non_null.update({
            "metadata.enrichment_status": "succeeded",
            "metadata.fetched_at": now,
        ...
        self.songs.update_one(
            {"id": song_id},
            {"$set": non_null, "$inc": {"metadata.enrichment_attempts": 1}},
        )
    ...
    def _best(self, song, candidates):
        ...
        def score(recording):
            artists = ", ".join(
                x.get("name", "")
                for x in recording.get("artist-credit", [])
                if isinstance(x, dict)
            )
            value = similarity(song.get("title"), recording.get("title")) * 0.55
            value += similarity(song.get("artist"), artists) * 0.35
            expected = int(song.get("duration") or 0)
            actual = int((recording.get("length") or 0) / 1000)
        ...
        credits = [
            x for x in recording.get("artist-credit", []) if isinstance(x, dict)
        ]
        ...
        if release.get("id"):
            try:
                ...
                log.warning(
                    "Could not load MusicBrainz release details for %s",
                    release.get("id"),
                )
        ...
        genres = sorted(
            {x.get("name") for x in recording.get("genres", []) if x.get("name")}
        )
        ...
        return {
            ...
            "release_date": date,
            "release_year": int(date[:4]) if date and date[:4].isdigit() else None,
            ...
            "recording_length": (recording.get("length") or 0) / 1000 or None,
